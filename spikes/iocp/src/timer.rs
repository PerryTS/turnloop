//! Timer probes: documented APC path and dynamically resolved NT wait packets.
use crate::port::{Port, TIMER, bool_result, owned};
use std::{
    cell::Cell,
    ffi::{CStr, c_void},
    io,
...
pub struct ApcTimer {
    timer: OwnedHandle,
    fired: Box<Cell<u64>>,
    // SetWaitableTimer queues the APC to the ARMING thread, so !Send + !Sync.
    thread_bound: PhantomData<Rc<()>>,
}

unsafe extern "system" fn fired(context: *const c_void, _: u32, _: u32) {
    // SAFETY: stable Cell belongs to this thread; timer cancellation removes queued
    // APCs before it is freed. Callback neither allocates nor invokes host code.
    let count = unsafe { &*context.cast::<Cell<u64>>() };
...
impl ApcTimer {
    pub fn new() -> io::Result<Self> {
        Ok(Self {
            ...
            fired: Box::new(Cell::new(0)),
            thread_bound: PhantomData,
        })
    }
    pub fn arm(&mut self, delay: Duration) -> io::Result<()> {
        let due = relative_due(delay);
        // SAFETY: timer and stable callback context live through cancellation/drop.
        // Negative due times are relative 100ns units, period zero is one-shot.
        ...
        bool_result(unsafe {
            SetWaitableTimer(
                ...
                0,
                Some(fired),
                (&*self.fired as *const Cell<u64>).cast(),
                0,
            )
        })
    }
    ...
    pub fn cancel(&mut self) -> io::Result<()> {
        // SAFETY: timer remains live; same-thread APC cannot execute concurrently.
        // CancelWaitableTimer cancels outstanding APCs (not just the deadline).
...
impl Drop for ApcTimer {
    fn drop(&mut self) {
        if self.cancel().is_err() {
            // Preserve callback storage if an unexpected cancellation error occurs.
            let cell = std::mem::replace(&mut self.fired, Box::new(Cell::new(0)));
            let _ = Box::leak(cell);
        }
    }
}
...
impl PacketTimer {
    ...
    pub unsafe fn dequeued(&mut self) {
        self.active = false;
    }
