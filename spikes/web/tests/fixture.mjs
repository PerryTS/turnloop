...
const stats={fetches:0,aborted:0,websockets:0,echoed:0};
const server=http.createServer(async(req,res)=>{
  ...
  else if(req.url==='/slow') { stats.fetches++; const timer=setTimeout(()=>res.end('slow'),10000); res.on('close',()=>{clearTimeout(timer);if(!res.writableEnded)stats.aborted++;}); }
