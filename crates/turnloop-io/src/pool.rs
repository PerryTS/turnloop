...
pub enum Action<I, L> {
    ...
    Close(I),
    Remove(I),
    Acquired { token: u64, lease: L },
    Failed { token: u64, error: io::Error },
    Ended,
}
...
pub trait Connector<B: Backend>: 'static {
    ...
    fn connect(
        ...
        deadline: Instant,
    ) -> impl Future<Output = io::Result<Self::Connection>> + 'static;
}
type Connecting<C> = Pin<Box<dyn Future<Output = io::Result<C>>>>;
struct Entry<B: Backend, I, C> {
    id: I,
    client: Option<C>,
    connecting: Option<Connecting<C>>,
    handle: Option<Handle>,
    closing: Option<turnloop::Close<B>>,
    closed: bool,
}
struct Waiter<L> {
    token: u64,
    result: Option<io::Result<L>>,
    waker: Option<Waker>,
}
struct State<B: Backend, P: Policy, C> {
    policy: P,
    entries: Vec<Entry<B, P::Id, C>>,
...
struct Owner<B: Backend, P: Policy, C> {
    state: Rc<RefCell<State<B, P, C>>>,
    executor: ExecutorHandle<B>,
    _task: JoinHandle<()>,
}
/// Cloneable pool. Checkout and release reuse retained slots after warm-up.
pub struct Pool<B: Backend, P: Policy, C> {
    owner: Rc<Owner<B, P, C>>,
}
impl<B: Backend, P: Policy, C> Clone for Pool<B, P, C> {
    fn clone(&self) -> Self {
        Self {
...
impl<B: Backend + 'static, P: Policy, C: Connection> Pool<B, P, C> {
    pub fn new<F: Connector<B, Connection = C>>(
        executor: &ExecutorHandle<B>,
        policy: P,
        connector: F,
                ...
                if let Err(e) = result {
                    ...
                    for entry in &state.entries {
                        if let Some(h) = entry.handle {
                            let _ = exec.driver().close(h, turnloop::Token(0));
                        }
                    }
                    state.entries.clear();
                    return Poll::Ready(());
    ...
    pub fn total(&self) -> usize {
        self.owner.state.borrow().entries.len()
    }
    ...
    pub async fn acquire(&self, at: Instant) -> io::Result<Lease<B, P, C>> {
        let token = {
            ...
            state.token = state
                .token
                .checked_add(1)
                .ok_or_else(|| io::Error::other("pool token exhausted"))?;
            let token = state.token;
            state
                .policy
                .checkout(token, self.owner.executor.now(), at)?;
        ...
        let result = crate::deadline(
            ...
            poll_fn(|cx| {
                ...
                let Some(index) = state.waiters.iter().position(|w| w.token == token) else {
                    return Poll::Ready(Err(io::Error::other("missing pool waiter")));
                };
                if let Some(result) = state.waiters[index].result.take() {
                    ...
                    return Poll::Ready(result.and_then(|lease| {
                        let entry = state
                            .entries
                            .iter_mut()
                            .find(|e| e.id == P::id(lease))
                            .ok_or_else(|| io::Error::other("missing pooled connection"))?;
                        let client = entry
                            .client
                            .take()
                            .ok_or_else(|| io::Error::other("connection already leased"))?;
                        Ok(Lease {
                            owner: self.owner.clone(),
                            client: Some(client),
                            lease,
    ...
    pub async fn end(&self) -> io::Result<()> {
        ...
        poll_fn(|cx| {
            ...
            if !state.end_wakers.iter().any(|w| w.will_wake(cx.waker())) {
                state.end_wakers.push(cx.waker().clone());
            }
            Poll::Pending
        })
        .await
...
fn drive<B: Backend, P: Policy, C: Connection, F: Connector<B, Connection = C>>(
    state: &mut State<B, P, C>,
    executor: &ExecutorHandle<B>,
    connector: &F,
    timeout: Duration,
    timer: &mut Option<(Instant, Sleep<B>)>,
    cx: &mut Context<'_>,
) -> io::Result<()> {
    ...
    loop {
        while let Some(action) = state.policy.event() {
            match action {
                Action::Connect(id) => state.entries.push(Entry {
                    id,
                    client: None,
                    connecting: Some(Box::pin(connector.connect(executor, now + timeout))),
                    handle: None,
                    closing: None,
                    closed: false,
                }),
                Action::Close(id) => {
                    if let Some(entry) = state.entries.iter_mut().find(|e| e.id == id) {
                        if let Some(h) = entry.handle {
                            // Register physical-close observation before dropping the
                            // client; its destructor may independently request close.
                            entry.closing = Some(executor.close(h));
                        }
                        entry.client = None;
                ...
                Action::Acquired { token, lease } => {
                    if let Some(w) = state.waiters.iter_mut().find(|w| w.token == token) {
                        w.result = Some(Ok(lease));
                        wake(&w.waker);
                    } else {
                        state.policy.checkin(lease, now, true)?;
                    }
                }
    ...
    fn drop(&mut self) {
        if !self.done {
            ...
            if let Some(i) = state.waiters.iter().position(|w| w.token == self.token) {
                if let Some(Ok(lease)) = state.waiters.swap_remove(i).result {
                    let _ = state.policy.checkin(lease, self.owner.executor.now(), true);
                } else {
                    state.policy.cancel(self.token, self.owner.executor.now());
                }
            }
            wake(&state.changed);
        }
...
pub struct Lease<B: Backend, P: Policy, C: Connection> {
    owner: Rc<Owner<B, P, C>>,
    client: Option<C>,
    lease: P::Lease,
}
impl<B: Backend, P: Policy, C: Connection> Deref for Lease<B, P, C> {
    type Target = C;
    fn deref(&self) -> &C {
        self.client.as_ref().expect("owned lease")
    }
}
impl<B: Backend, P: Policy, C: Connection> DerefMut for Lease<B, P, C> {
    fn deref_mut(&mut self) -> &mut C {
        self.client.as_mut().expect("owned lease")
    }
}
