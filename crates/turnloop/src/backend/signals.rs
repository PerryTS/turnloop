...
use std::{os::fd::{AsRawFd, FromRawFd, OwnedFd}, sync::{Arc, Mutex, OnceLock, Weak, atomic::{AtomicBool, Ordering}}};

pub(super) fn number(signal: Signal) -> Result<i32> {
    Ok(match signal {
        Signal::Int => libc::SIGINT, Signal::Term => libc::SIGTERM,
        Signal::Kill => libc::SIGKILL, Signal::Hup => libc::SIGHUP,
        Signal::Chld => libc::SIGCHLD, Signal::WinCh => libc::SIGWINCH,
        Signal::Usr1 => libc::SIGUSR1, Signal::Usr2 => libc::SIGUSR2,
...
impl Ticket {
    ...
    pub fn ready(&self) -> bool { self.pending.load(Ordering::Acquire) }
}
struct Record {
    number: i32,
...
fn change(fd: i32, signal: i32, flags: u16) -> Result<()> {
    let event = libc::kevent { ident: signal as usize, filter: libc::EVFILT_SIGNAL, flags, fflags: 0, data: 0, udata: std::ptr::null_mut() };
    // SAFETY: valid kqueue and initialized signal change, no event output or wait.
    if unsafe { libc::kevent(fd, &event, 1, std::ptr::null_mut(), 0, std::ptr::null()) } < 0 { return Err(last_error()); }
    Ok(())
}
fn start() -> Result<Arc<Dispatcher>> {
    ...
    let fd = {
        ...
        if unsafe { libc::fcntl(raw, libc::F_SETFD, libc::FD_CLOEXEC) } < 0 { return Err(last_error()); }
        fd
    };
    #[cfg(turnloop_backend = "epoll")]
    let (fd, writer) = {
        ...
        if unsafe { libc::pipe2(pair.as_mut_ptr(), libc::O_CLOEXEC | libc::O_NONBLOCK) } < 0 { return Err(last_error()); }
        // SAFETY: successful pipe2 returned two exclusively owned descriptors.
        let (reader, writer) = unsafe { (OwnedFd::from_raw_fd(pair[0]), OwnedFd::from_raw_fd(pair[1])) };
        WRITE_FD.store(writer.as_raw_fd(), Ordering::Release);
        (reader, writer)
    };
    let dispatcher = Arc::new(Dispatcher { fd, records: Mutex::new(Vec::new()), #[cfg(turnloop_backend = "epoll")] _writer: writer });
    let d = dispatcher.clone();
    std::thread::Builder::new().name("turnloop-signals".into()).spawn(move || d.run()).map_err(Error::from)?;
    Ok(dispatcher)
}
impl Dispatcher {
    ...
    fn run(&self) {
        loop {
            #[cfg(turnloop_backend = "kqueue")]
            {
                ...
                let n = unsafe { libc::kevent(self.fd.as_raw_fd(), std::ptr::null(), 0, &mut event, 1, std::ptr::null()) };
                if n == 1 { self.dispatch(event.ident as i32); }
                else if n < 0 && last_error().os != Some(libc::EINTR) { return; }
            }
            #[cfg(turnloop_backend = "epoll")]
            {
                let mut fd = libc::pollfd { fd: self.fd.as_raw_fd(), events: libc::POLLIN, revents: 0 };
                // SAFETY: helper-owned pollfd, one record, unbounded wait.
                let n = unsafe { libc::poll(&mut fd, 1, -1) };
                if n < 0 { if last_error().os == Some(libc::EINTR) { continue; } return; }
                ...
                for (sig, pending) in PENDING.iter().enumerate() {
                    if pending.swap(false, Ordering::AcqRel) { self.dispatch(sig as i32); }
                }
            }
        }
    }
...
pub(super) fn subscribe(signal: Signal, notifier: Notifier) -> Result<Subscription> {
    ...
    if signal == Signal::Kill { return Err(Error::new(ErrorKind::InvalidInput)); }
    let dispatcher = DISPATCHER.get_or_init(start).as_ref().map_err(|&e| e)?;
    let mut records = dispatcher.records.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
    let ticket = Arc::new(Ticket { signal, pending: AtomicBool::new(false), notifier });
    if let Some(record) = records.iter_mut().find(|r| r.number == number) {
        ...
        let (mut action, mut original): (libc::sigaction, libc::sigaction) = unsafe { std::mem::zeroed() };
        action.sa_sigaction = handler as *const () as usize;
        action.sa_flags = libc::SA_RESTART | if signal == Signal::Chld { libc::SA_NOCLDSTOP } else { 0 };
        ...
        if unsafe { libc::sigaction(number, &action, &mut original) } < 0 { return Err(last_error()); }
        #[cfg(turnloop_backend = "kqueue")]
        if let Err(e) = change(dispatcher.fd.as_raw_fd(), number, libc::EV_ADD | libc::EV_CLEAR) {
            // SAFETY: restore precisely the action captured above on registration failure.
            unsafe { libc::sigaction(number, &original, std::ptr::null_mut()); }
            return Err(e);
        }
        records.push(Record { number, original, subscribers: vec![Arc::downgrade(&ticket)] });
    }
...
impl std::ops::Deref for Subscription {
    type Target = Ticket;
    fn deref(&self) -> &Ticket { &self.0 }
}
impl Drop for Subscription {
    fn drop(&mut self) {
        let Some(Ok(dispatcher)) = DISPATCHER.get() else { return; };
        let Ok(number) = number(self.signal) else { return; };
        let mut records = dispatcher.records.lock().unwrap_or_else(std::sync::PoisonError::into_inner);
        if let Some(i) = records.iter().position(|r| r.number == number) {
            records[i].subscribers.retain(|w| w.as_ptr() != Arc::as_ptr(&self.0));
            if records[i].subscribers.is_empty() {
                ...
                unsafe { libc::sigaction(number, &record.original, std::ptr::null_mut()); }
            }
        }
    }
}
