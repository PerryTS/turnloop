...
unsafe extern "C" {
    #[link_name="[waitable-set-new]"] fn set_new()->u32;
    #[link_name="[waitable-set-drop]"] fn set_drop(set:u32);
    #[link_name="[waitable-join]"] fn join(waitable:u32,set:u32);
    #[link_name="[waitable-set-wait]"] fn wait(set:u32,payload:*mut [u32;2])->u32;
    #[link_name="[waitable-set-poll]"] fn poll(set:u32,payload:*mut [u32;2])->u32;
    #[link_name="[subtask-cancel]"] pub(super) fn subtask_cancel(task:u32)->u32;
    #[link_name="[subtask-drop]"] pub(super) fn subtask_drop(task:u32);
}
...
impl WaitSet {
    ...
    pub fn step(&self,blocking:bool)->(u32,u32,u32){
        ...
        let kind=unsafe {if blocking {wait(self.0,&mut payload)}else{poll(self.0,&mut payload)}};
        (kind,payload[0],payload[1])
    }
}
impl Drop for WaitSet {fn drop(&mut self){
    // SAFETY: backend cancels all outstanding operations before dropping its set.
    unsafe {set_drop(self.0);}
}}
