...
impl Driver {
    fn connect(port: u16, config: Config) -> Self {
        ...
        core.connect(Instant::now()).expect("fixture operation must succeed");
        assert_eq!(core.poll_event(), Some(Event::Connect));
        core.transport_connected().expect("fixture operation must succeed");
        let socket: Box<dyn Socket> = if tls {
            ...
            let mut tls = rustls::StreamOwned::new(
                rustls::ClientConnection::new(Arc::new(config), "localhost".try_into().expect("fixture operation must succeed"))
                    .expect("fixture operation must succeed"),
                stream,
            );
            ...
            core.tls_established().expect("fixture operation must succeed");
            Box::new(tls)
        } else {
            Box::new(stream)
        };
    ...
    fn event(&mut self) -> Event {
        loop {
            ...
            let n = self.socket.read(&mut bytes).expect("fixture operation must succeed");
            assert!(n > 0, "server EOF");
            self.core.receive(&bytes[..n]).expect("fixture operation must succeed");
        }
    }
    fn flush(&mut self) {
        self.socket.write_all(self.core.output()).expect("fixture operation must succeed");
    ...
    fn query(&mut self, args: &[&[u8]]) -> Result<Value, turnloop_redis::Error> {
        self.id += 1;
        self.core.command(self.id, args, None).expect("fixture operation must succeed");
        match self.event() {
...
fn auth() -> Config {
    Config {
        ...
        password: Some(std::env::var("TURNLOOP_TEST_REDIS_PASSWORD").expect("fixture operation must succeed")),
        database: 2,
...
fn real_redis_commands_transactions_pubsub_reconnect_tls() {
    ...
    assert_eq!(
        d.query(&[b"CLIENT", b"GETNAME"]).expect("fixture operation must succeed").bytes(),
        Some(b"turnloop-lane".as_slice())
    );
    assert_eq!(
        d.query(&[b"SET", b"binary", b"a\0\xff"]).expect("fixture operation must succeed").bytes(),
        Some(b"OK".as_slice())
    );
    assert_eq!(
        d.query(&[b"GET", b"binary"]).expect("fixture operation must succeed").bytes(),
        Some(b"a\0\xff".as_slice())
    );
    assert_eq!(d.query(&[b"GET", b"missing"]).expect("fixture operation must succeed"), Value::Null);
    for (id, args) in [
        (100, vec![b"INCR".as_slice(), b"counter"]),
        (101, vec![b"INCR".as_slice(), b"counter"]),
        (102, vec![b"GET".as_slice(), b"counter"]),
    ] {
        d.core.command(id, &args, None).expect("fixture operation must succeed");
    }
    assert!(matches!(
        d.event(),
        Event::Reply {
    ...
    assert_eq!(
        d.query(&[b"MULTI"]).expect("fixture operation must succeed").bytes(),
        Some(b"OK".as_slice())
    );
    assert_eq!(
        d.query(&[b"SET", b"tx", b"committed"]).expect("fixture operation must succeed").bytes(),
    ...
    d.query(&[b"MULTI"]).expect("fixture operation must succeed");
    d.query(&[b"SET", b"tx", b"discarded"]).expect("fixture operation must succeed");
    d.query(&[b"DISCARD"]).expect("fixture operation must succeed");
    assert_eq!(
        d.query(&[b"GET", b"tx"]).expect("fixture operation must succeed").bytes(),
        Some(b"committed".as_slice())
    );
    let mut other = Driver::connect(port("TURNLOOP_TEST_REDIS_PORT"), auth());
    d.query(&[b"WATCH", b"tx"]).expect("fixture operation must succeed");
    other.query(&[b"SET", b"tx", b"conflict"]).expect("fixture operation must succeed");
    d.query(&[b"MULTI"]).expect("fixture operation must succeed");
    d.query(&[b"SET", b"tx", b"wrong"]).expect("fixture operation must succeed");
    assert_eq!(d.query(&[b"EXEC"]).expect("fixture operation must succeed"), Value::Null);
    assert_eq!(
        d.query(&[b"HSET", b"hash", b"a", b"b"]).expect("fixture operation must succeed"),
        Value::Integer(1)
    );
    assert!(
        matches!(d.query(&[b"HGETALL", b"hash"]).expect("fixture operation must succeed"), Value::Map(v) if v.len() == 1 && v[0].0.bytes() == Some(b"a"))
    );
    d.query(&[b"SADD", b"set", b"x", b"y"]).expect("fixture operation must succeed");
    assert!(matches!(d.query(&[b"SMEMBERS", b"set"]).expect("fixture operation must succeed"), Value::Set(v) if v.len() == 2));
    let error = d.query(&[b"GET", b"hash"]).unwrap_err();
    assert_eq!(error.name, "ReplyError");
    assert!(error.message.starts_with("WRONGTYPE"));
    assert_eq!(
        d.query(&[b"BLPOP", b"queue", b"0.05"]).expect("fixture operation must succeed"),
        Value::Null
    );
    d.core
        .command(
        ...
        .expect("fixture operation must succeed");
    d.flush();
    other.query(&[b"LPUSH", b"queue", b"pushed"]).expect("fixture operation must succeed");
    assert!(
        matches!(d.event(), Event::Reply { token: 500, result: Ok(Value::Array(v)) } if v[1].bytes() == Some(b"pushed"))
    );
    assert_eq!(
        d.query(&[b"SUBSCRIBE", b"news", b"alerts"]).expect("fixture operation must succeed"),
        Value::Integer(2)
    );
    assert_eq!(
        d.query(&[b"PSUBSCRIBE", b"news*"]).expect("fixture operation must succeed"),
        Value::Integer(3)
    );
    assert!(d.core.command(501, &[b"GET", b"tx"], None).is_err());
    assert_eq!(
        other.query(&[b"PUBLISH", b"news", b"payload"]).expect("fixture operation must succeed"),
        Value::Integer(2)
    );
    ...
    let now = Instant::now();
    d.core.retry(now, Some(Duration::ZERO)).expect("fixture operation must succeed");
    d.core.handle_timeout(now);
    assert_eq!(d.event(), Event::Connect);
    let stream = TcpStream::connect(("127.0.0.1", port("TURNLOOP_TEST_REDIS_PORT"))).expect("fixture operation must succeed");
    stream
        .set_read_timeout(Some(Duration::from_secs(5)))
        .expect("fixture operation must succeed");
    d.socket = Box::new(stream);
    d.core.transport_connected().expect("fixture operation must succeed");
    assert!(matches!(d.event(), Event::Ready { .. }));
    other.query(&[b"PUBLISH", b"news", b"again"]).expect("fixture operation must succeed");
    assert!(matches!(d.event(), Event::Message { payload, .. } if payload == b"again"));
    d.event();
    assert_eq!(d.query(&[b"UNSUBSCRIBE"]).expect("fixture operation must succeed"), Value::Integer(1));
    assert_eq!(d.query(&[b"PUNSUBSCRIBE"]).expect("fixture operation must succeed"), Value::Integer(0));
    assert_eq!(
        d.query(&[b"CLIENT", b"GETNAME"]).expect("fixture operation must succeed").bytes(),
        Some(b"turnloop-lane".as_slice())
    );
    ...
    assert_eq!(
        tls.query(&[b"GET", b"binary"]).expect("fixture operation must succeed").bytes(),
        Some(b"a\0\xff".as_slice())
    );
    assert_eq!(
        tls.query(&[b"QUIT"]).expect("fixture operation must succeed").bytes(),
        Some(b"OK".as_slice())
    );
...
fn real_cluster_slots_shards_moved_ask_and_sentinel() {
    ...
    map.update_slots(&seed.query(&[b"CLUSTER", b"SLOTS"]).expect("fixture operation must succeed"), "127.0.0.1")
        .expect("fixture operation must succeed");
    for slot in 0..16384 {
        assert!(map.endpoint(slot).is_some());
    }
    map.update_shards(&seed.query(&[b"CLUSTER", b"SHARDS"]).expect("fixture operation must succeed"), "127.0.0.1")
        .expect("fixture operation must succeed");
    let key = (0..10000)
        .map(|i| format!("key{i}"))
        .find(|k| map.endpoint(key_slot(k.as_bytes())).expect("fixture operation must succeed").port != port("TURNLOOP_TEST_REDIS_CLUSTER_PORT"))
        .expect("fixture operation must succeed");
    ...
    owner.query(&[b"SET", key.as_bytes(), b"routed"]).expect("fixture operation must succeed");
    assert_eq!(
        owner.query(&[b"GET", key.as_bytes()]).expect("fixture operation must succeed").bytes(),
    ...
    let endpoint = map.route_keys(&tags).expect("fixture operation must succeed").expect("fixture operation must succeed");
    let mut same = Driver::connect(endpoint.port, Config::default());
    same.query(&[b"MSET", tags[0], b"one", tags[1], b"two"])
        .expect("fixture operation must succeed");
    assert_eq!(
        same.query(&[b"MGET", tags[0], tags[1]]).expect("fixture operation must succeed"),
        Value::Array(vec![
    ...
    owner.query(&[b"DEL", key.as_bytes()]).expect("fixture operation must succeed");
    let slot = key_slot(key.as_bytes()).to_string();
    let source_id = owner.query(&[b"CLUSTER", b"MYID"]).expect("fixture operation must succeed");
    let dest_id = seed.query(&[b"CLUSTER", b"MYID"]).expect("fixture operation must succeed");
    seed.query(&[
        b"CLUSTER",
        b"SETSLOT",
        slot.as_bytes(),
    ...
    owner
        ...
        .expect("fixture operation must succeed");
    let ask = Redirect::parse(&owner.query(&[b"GET", key.as_bytes()]).unwrap_err()).expect("fixture operation must succeed");
    assert!(ask.asking);
    ...
    assert_eq!(map.endpoint(ask.slot).expect("fixture operation must succeed").port, redirect.endpoint.port);
    seed.query(&[b"ASKING"]).expect("fixture operation must succeed");
    assert_eq!(seed.query(&[b"GET", key.as_bytes()]).expect("fixture operation must succeed"), Value::Null);
    owner
    ...
    discovery
        .reply(&discovered.query(&[b"ROLE"]).expect("fixture operation must succeed"))
        .expect("fixture operation must succeed");
    assert_eq!(
    ...
    assert_eq!(
        discovered.query(&[b"PING"]).expect("fixture operation must succeed").bytes(),
