...
fn codec_fragments_all_types_and_limits() {
    ...
    for frame in frames {
        for end in 0..frame.len() {
            assert_eq!(
                resp::decode(&frame[..end], Limits::default()).unwrap(),
                None,
                "{frame:?} at {end}"
            );
        }
        assert_eq!(
            resp::decode(frame, Limits::default()).unwrap().unwrap().1,
            frame.len()
        );
    }
    for bad in [
...
fn ordered_pipeline_timeout_tombstone_and_close() {
    ...
    c.command(1, &[b"BLPOP", b"queue", b"0"], Some(now))
        .unwrap();
    c.command(2, &[b"INCR", b"counter"], None).unwrap();
    c.consume_output(3);
    ...
    c.command(3, &[b"PING"], None).unwrap();
    c.close();
    c.close();
    assert!(matches!(
        c.poll_event(),
...
fn reconnect_offline_order_resubscribe_and_retry_stop() {
    ...
    c.retry(now, Some(Duration::from_millis(50))).unwrap();
    ...
    c.poll_event();
    c.poll_event();
    c.retry(now, None).unwrap();
    assert_eq!(c.state(), State::Closed);
    let mut c = ready(Config::default());
    c.command(7, &[b"INCR", b"x"], None).unwrap();
    c.transport_lost();
    c.poll_event();
    c.poll_event();
    c.command(8, &[b"GET", b"x"], None).unwrap();
    c.retry(now, Some(Duration::ZERO)).unwrap();
    c.handle_timeout(now);
    c.poll_event();
    c.transport_connected().unwrap();
    c.consume_output(c.output().len());
...
fn cluster_hashes_and_redirects() {
    ...
    let redirect = Redirect::parse(&e).unwrap();
    assert_eq!(redirect.endpoint.host, "::1");
    let mut map = SlotMap::new();
    map.apply_redirect(&redirect).unwrap();
    assert_eq!(map.endpoint(123).unwrap().port, 32100);
    let ask = Redirect {
        asking: true,
...
fn pipelined_subscribe_unsubscribe_all_waits_for_every_ack() {
    let mut c = ready(Config::default());
    c.command(1, &[b"SUBSCRIBE", b"a", b"b"], None).unwrap();
    c.command(2, &[b"UNSUBSCRIBE"], None).unwrap();
    c.consume_output(c.output().len());
    c.receive(b">3\r\n+subscribe\r\n+a\r\n:1\r\n>3\r\n+subscribe\r\n+b\r\n:2\r\n>3\r\n+unsubscribe\r\n+a\r\n:1\r\n").unwrap();
    assert_eq!(
        c.poll_event(),
    ...
    c.command(3, &[b"GET", b"a"], None).unwrap();
    let mut c = ready(Config::default());
    c.command(4, &[b"SUBSCRIBE"], None).unwrap();
    c.consume_output(c.output().len());
    c.receive(b"-ERR wrong number of arguments\r\n").unwrap();
    c.poll_event();
    c.command(5, &[b"GET", b"a"], None).unwrap();
}
#[test]
fn sentinel_seed_iteration_stale_master_retry_and_redirect_budget() {
    ...
    let mut discovery =
        SentinelDiscovery::new(vec![endpoint.clone(), endpoint.clone()], "group".into()).unwrap();
    assert!(matches!(
        discovery.poll_action(),
        Some(DiscoveryAction::QueryMaster { .. })
    ));
    discovery.reply(&Value::Null).unwrap();
    assert!(matches!(
        discovery.poll_action(),
        Some(DiscoveryAction::QueryMaster { .. })
    ));
    ...
    let mut map = SlotMap::new();
    assert!(tracker.follow(&e, &mut map).unwrap().is_some());
    assert!(
        tracker
            .follow(&e, &mut map)
            .unwrap_err()
