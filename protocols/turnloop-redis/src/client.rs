...
use turnloop_io::{AsyncIo, Backend, Driver, ExecutorHandle, Instant, Output, SansIo, deadline};
...
impl Output for Core {
    ...
    fn consume_output(&mut self, n: usize) -> io::Result<()> {
        self.0.consume_output(n);
        Ok(())
    }
}
impl SansIo for Core {
    type Event<'a> = Event;
    fn event(&mut self, mut receive: impl FnMut(Event) -> io::Result<()>) -> io::Result<bool> {
        if let Some(e) = self.0.poll_event() {
            receive(e)?;
            Ok(true)
        } else {
            Ok(false)
    ...
    fn ingest(&mut self, bytes: &[u8], _: Instant) -> io::Result<usize> {
        self.0.receive(bytes).map_err(io::Error::other)?;
        Ok(bytes.len())
    }
    fn disconnected(&mut self) {
        self.0.transport_lost();
    }
}
...
impl<B: Backend> Client<B> {
    pub async fn connect(
        ...
        core.connect(time(executor.now())?)
            .map_err(io::Error::other)?;
        if !matches!(core.poll_event(), Some(Event::Connect)) {
            return Err(io::Error::other("missing Redis connect action"));
        }
        ...
        let mut client = Self {
            ...
            driver: Driver::new(Transport::Plain(stream), Core(core)),
    ...
    pub fn is_connected(&self) -> bool {
        self.driver.is_connected() && self.driver.core().0.state() == State::Ready
    }
    pub fn reconnect_count(&self) -> u64 {
        self.reconnects
    }
    pub fn close(&mut self) {
        self.driver.core_mut().0.close();
        self.driver.abort();
    }
    async fn reconnect(&mut self, at: Instant) -> io::Result<()> {
        loop {
            while let Some(event) = self.driver.core_mut().0.poll_event() {
                match event {
                    Event::Retry { attempt } => {
                        let delay = if attempt <= self.options.max_reconnects {
                        ...
                        self.driver
                            .core_mut()
                            .0
                            .retry(time(self.executor.now())?, delay)
                            .map_err(io::Error::other)?;
                    }
                    Event::Connect => {
                        match deadline(&self.executor, at, async {
                            self.executor
                                .connect(self.options.address, Default::default())
                                .await
                                .map_err(turnloop_io::error)
                        })
                        .await
            ...
            let until = self
                ...
                .0
                .next_timeout()
                .ok_or_else(|| io::Error::other("Redis reconnect has no deadline"))?;
            ...
            self.driver
                .core_mut()
                .0
                .handle_timeout(time(self.executor.now())?);
        }
    }
    async fn next_event(&mut self, at: Instant) -> io::Result<Event> {
        loop {
            ...
            let result = deadline(
                &self.executor,
                at,
                self.driver.next(&self.executor, |e| {
                    event = Some(e);
                    Ok(())
                }),
            )
            .await;
            if let Err(e) = result {
                if e.kind() == io::ErrorKind::TimedOut || self.options.max_reconnects == 0 {
                    return Err(e);
                }
                self.reconnect(at).await?;
                continue;
    ...
    pub async fn pipeline(
        ...
        commands: &[&[&[u8]]],
        at: Instant,
        mut receive: impl FnMut(usize, Result<Value, crate::Error>) -> io::Result<()>,
    ) -> io::Result<()> {
        ...
        if !operation.client.is_connected() {
            while !operation.client.is_connected() {
                operation.client.next_event(at).await?;
            }
        }
        let first = operation
            .client
            .token
            .checked_add(1)
            .ok_or_else(|| io::Error::other("Redis token exhausted"))?;
        for command in commands {
            operation.client.token = operation
                .client
                .token
                .checked_add(1)
                .ok_or_else(|| io::Error::other("Redis token exhausted"))?;
            ...
            operation
                ...
                .core_mut()
                .0
                .command(token, command, Some(time(at)?))
                .map_err(io::Error::other)?;
        }
        let mut replies = 0;
        while replies < commands.len() {
            match operation.client.next_event(at).await? {
                Event::Reply { token, result } => {
                    if token != first + replies as u64 {
                        return Err(io::Error::other("Redis pipeline reply order"));
                    }
                    receive(replies, result)?;
                    replies += 1;
                }
                e @ (Event::Message { .. } | Event::Push(_)) => {
                    operation.client.messages.push_back(e)
                }
                Event::Closed => return Err(io::ErrorKind::NotConnected.into()),
                _ => {}
            }
    ...
    pub async fn command(&mut self, args: &[&[u8]], at: Instant) -> io::Result<Value> {
        let mut result = None;
        self.pipeline(&[args], at, |_, reply| {
            result = Some(reply);
            Ok(())
        })
        .await?;
        result
            .ok_or_else(|| io::Error::other("missing Redis reply"))?
            .map_err(io::Error::other)
    }
...
pub struct Subscriber<'a, B: Backend> {
    client: &'a mut Client<B>,
}
impl<B: Backend> Subscriber<'_, B> {
    pub async fn next(&mut self, at: Instant) -> io::Result<Event> {
        if let Some(event) = self.client.messages.pop_front() {
            return Ok(event);
        }
        loop {
            match self.client.next_event(at).await? {
                e @ (Event::Message { .. } | Event::Push(_)) => return Ok(e),
                Event::Closed => return Err(io::ErrorKind::NotConnected.into()),
                _ => {}
            }
        }
...
impl<B: Backend> Drop for Subscriber<'_, B> {
    fn drop(&mut self) {
        self.client.close();
    }
}
