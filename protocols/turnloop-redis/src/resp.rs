...
impl Value {
    ...
    pub fn items(&self) -> Option<&[Value]> {
        match self {
            Self::Array(v) | Self::Set(v) | Self::Push(v) => Some(v),
            _ => None,
        }
    }
...
fn number(v: &[u8]) -> Result<i64, DecodeError> {
    if v.is_empty()
        ...
        || v.iter()
            .enumerate()
            .any(|(i, b)| !b.is_ascii_digit() && !(i == 0 && *b == b'-'))
    {
        return Err(DecodeError::Invalid);
    }
    std::str::from_utf8(v)
...
fn node(
    ...
    if depth > limits.depth || *budget == 0 {
    ...
    *p += 1;
    ...
    Ok(match kind {
        ...
        b'$' | b'!' | b'=' => {
            if head == b"?" {
                return Err(DecodeError::StreamingUnsupported);
            }
            let len = number(head)?;
            if len == -1 && kind == b'$' {
        ...
        b'*' | b'~' | b'>' | b'%' | b'|' => {
            ...
            let total = count
                .checked_mul(if pairs { 2 } else { 1 })
                .ok_or(DecodeError::Limit)?;
            if total > *budget {
                return Err(DecodeError::Limit);
            }
            let mut values = if build && !pairs {
                Vec::with_capacity(count)
            } else {
                Vec::new()
            };
            let mut entries = if build && pairs {
                Vec::with_capacity(count)
            } else {
                Vec::new()
            };
            ...
            match kind {
                ...
                _ => {
                    ...
                    if build {
                        Value::Attribute(entries, Box::new(value))
                    } else {
                        Value::Null
                    }
                }
...
/// Validate and locate a complete frame without allocating its result.
pub fn frame_len(input: &[u8], limits: Limits) -> Result<Option<usize>, DecodeError> {
    let mut end = 0;
    match node(input, &mut end, 0, &mut { limits.items }, limits, false) {
        Err(DecodeError::Incomplete) if input.len() <= limits.bytes => return Ok(None),
        Err(DecodeError::Incomplete) => return Err(DecodeError::Limit),
        Err(e) => return Err(e),
        Ok(_) => {}
    }
    if end > limits.bytes {
...
pub fn decode(input: &[u8], limits: Limits) -> Result<Option<(Value, usize)>, DecodeError> {
    let Some(end) = frame_len(input, limits)? else {
        return Ok(None);
    };
    let mut pos = 0;
    let value = node(input, &mut pos, 0, &mut { limits.items }, limits, true)?;
    Ok(Some((value, end)))
}
...
/// Borrowed pub/sub message fields; used before general materialization so
/// routing messages allocates only their returned channel/pattern/payload.
...
pub fn message(
    input: &[u8],
    limits: Limits,
) -> Result<Option<(MessageRef<'_>, usize)>, DecodeError> {
    ...
    let mut pos = 1;
    let count = number(line(input, &mut pos)?)?;
    if count != 3 && count != 4 {
        return Ok(None);
    }
    fn bytes<'a>(input: &'a [u8], pos: &mut usize) -> Option<&'a [u8]> {
        let kind = *input.get(*pos)?;
        *pos += 1;
        let head = line(input, pos).ok()?;
        match kind {
            b'+' => Some(head),
            b'$' => {
                let len = usize::try_from(number(head).ok()?).ok()?;
                let data = input.get(*pos..pos.checked_add(len)?)?;
                *pos += len + 2;
                Some(data)
            }
            _ => None,
    ...
    let tag = bytes(input, &mut pos);
    let pattern = match (count, tag) {
        (3, Some(b"message")) => None,
        (4, Some(b"pmessage")) => Some(bytes(input, &mut pos).ok_or(DecodeError::Invalid)?),
        _ => return Ok(None),
    };
    let channel = bytes(input, &mut pos).ok_or(DecodeError::Invalid)?;
    let payload = bytes(input, &mut pos).ok_or(DecodeError::Invalid)?;
    Ok(Some((
        MessageRef {
            pattern,
            channel,
