...
use crate::{
    resp::Value,
    routing::{DiscoveryAction, Endpoint, Redirect, SentinelDiscovery, SlotMap},
};
use std::io;
use turnloop_io::{Backend, ExecutorHandle, Instant};
pub struct ClusterClient<B: Backend> {
    executor: ExecutorHandle<B>,
    options: ConnectOptions,
    slots: SlotMap,
    nodes: Vec<(Endpoint, Client<B>)>,
...
impl<B: Backend> ClusterClient<B> {
    pub async fn connect(
        ...
        let seed = Endpoint {
            host: options.address.ip().to_string(),
    ...
    pub async fn refresh(&mut self, at: Instant) -> io::Result<()> {
        ...
        self.slots
            .update_slots(&value, &self.nodes[0].0.host)
            .map_err(io::Error::other)
    }
    async fn node(&mut self, endpoint: Endpoint, at: Instant) -> io::Result<usize> {
        if let Some(i) = self.nodes.iter().position(|(e, _)| *e == endpoint) {
            return Ok(i);
        }
        let mut options = self.options.clone();
        options.address =
            turnloop_io::resolve(&self.executor, &endpoint.host, endpoint.port, at).await?;
        let client = Client::connect(&self.executor, &options, at).await?;
        self.nodes.push((endpoint, client));
        Ok(self.nodes.len() - 1)
    }
    ...
    pub async fn command(&mut self, args: &[&[u8]], at: Instant) -> io::Result<Value> {
        ...
        let mut index = match endpoint {
            Some(e) => match self.nodes.iter().position(|(n, _)| n == e) {
                Some(i) => i,
                None => {
                    let e = e.clone();
                    self.node(e, at).await?
                }
            },
            None => 0,
        };
            ...
            match self.nodes[index].1.command(args, at).await {
                ...
                Err(error) => {
                    ...
                    if redirect.asking {
                        ...
                        if value.bytes() != Some(b"OK") {
                            return Err(io::Error::other("ASKING refused"));
                        }
                    }
                }
            }
...
pub async fn sentinel<B: Backend>(
    executor: &ExecutorHandle<B>,
    seeds: Vec<Endpoint>,
    name: String,
    sentinel_options: &ConnectOptions,
    data_options: &ConnectOptions,
    at: Instant,
) -> io::Result<Client<B>> {
    ...
    loop {
        ...
        match discovery.poll_action() {
            Some(DiscoveryAction::QueryMaster { sentinel, name }) => {
                ...
                let result = async {
                    options.address =
                        turnloop_io::resolve(executor, &sentinel.host, sentinel.port, at).await?;
                    let mut client = Client::connect(executor, &options, at).await?;
                    client
                        .command(
                            &[b"SENTINEL", b"get-master-addr-by-name", name.as_bytes()],
                            at,
                        )
                        .await
                }
                .await;
                match result {
                    Ok(reply) => discovery.reply(&reply).map_err(io::Error::other)?,
            ...
            Some(DiscoveryAction::VerifyRole {
                ...
                let result = async {
                    options.address =
                        turnloop_io::resolve(executor, &endpoint.host, endpoint.port, at).await?;
                    let mut client = Client::connect(executor, &options, at).await?;
                ...
                match result {
                    Ok((client, reply)) => {
                        discovery.reply(&reply).map_err(io::Error::other)?;
                        candidate = Some(client);
                    }
                    Err(_) => discovery.failed(),
                }
            }
            Some(DiscoveryAction::Discovered(_)) => {
                return candidate.ok_or_else(|| io::Error::other("missing Sentinel candidate"));
            }
            Some(DiscoveryAction::Retry { attempt }) => {
                let delay = if attempt <= sentinel_options.max_reconnects {
                    Some(sentinel_options.retry_delay)
                } else {
                    None
                };
                discovery
                    .retry(super::client::time(executor.now())?, delay)
                    .map_err(io::Error::other)?;
            }
            Some(DiscoveryAction::Stopped) => {
                return Err(io::Error::other("Sentinel discovery exhausted"));
            }
            None => {
                let until = discovery
                    .next_timeout()
                    .ok_or_else(|| io::Error::other("Sentinel has no pending deadline"))?;
