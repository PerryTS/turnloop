...
impl rustls::time_provider::TimeProvider for SuppliedTime {
    fn current_time(&self) -> Option<UnixTime> {
        Some(UnixTime::since_unix_epoch(Duration::from_secs(self.0.load(Ordering::Relaxed))))
    }
}
...
impl Default for ClientOptions {
    fn default() -> Self {
        Self { alpn: vec![b"http/1.1".to_vec()], ca: None, extra_ca_pem: Vec::new(),
            reject_unauthorized: true, enable_sni: true }
    }
}

#[derive(Clone)]
pub struct ClientConfig { config: Arc<rustls::ClientConfig>, time: Arc<SuppliedTime> }
impl ClientConfig {
    pub fn new(options: ClientOptions, unix_seconds: u64) -> Result<Self, Error> {
        ...
        if !options.reject_unauthorized {
            config.dangerous().set_certificate_verifier(Arc::new(Unverified { provider }));
        }
        config.alpn_protocols = options.alpn;
        config.enable_sni = options.enable_sni;
        Ok(Self { config: Arc::new(config), time })
    }
    ...
    pub fn connect(&self, name: ServerName<'static>) -> Result<Client, Error> {
        Ok(Client { inner: rustls::client::UnbufferedClientConnection::new(self.config.clone(), name)?,
            time: self.time.clone(), deadline: None, timed_out: false })
    }
}

#[derive(Clone)]
pub struct ServerConfig { config: Arc<rustls::ServerConfig>, time: Arc<SuppliedTime> }
impl ServerConfig {
    pub fn new(chain: Vec<CertificateDer<'static>>, key: PrivateKeyDer<'static>,
        alpn: Vec<Vec<u8>>, unix_seconds: u64) -> Result<Self, Error> {
        let time = Arc::new(SuppliedTime(AtomicU64::new(unix_seconds)));
        let mut config = rustls::ServerConfig::builder_with_details(
            Arc::new(rustls::crypto::ring::default_provider()), time.clone())
    ...
    pub fn accept(&self) -> Result<Server, Error> {
        Ok(Server { inner: rustls::server::UnbufferedServerConnection::new(self.config.clone())?,
            time: self.time.clone(), deadline: None, timed_out: false })
    }
}
...
macro_rules! endpoint {
    ($name:ident, $connection:ty, $data:ty) => {
        pub struct $name { inner: $connection, time: Arc<SuppliedTime>, deadline: Option<Instant>, timed_out: bool }
        impl $name {
            /// No input/output staging copies are introduced by this wrapper.
            pub fn process<'c, 'i>(&'c mut self, incoming: &'i mut [u8], unix_seconds: u64)
                -> UnbufferedStatus<'c, 'i, $data> {
                self.time.0.store(unix_seconds, Ordering::Relaxed);
                if self.timed_out { return UnbufferedStatus { discard: 0,
                    state: Err(Error::General("TLS handshake timeout".into())) }; }
                self.inner.process_tls_records(incoming)
            }
            pub fn alpn_protocol(&self) -> Option<&[u8]> { self.inner.alpn_protocol() }
            pub fn is_handshaking(&self) -> bool { self.inner.is_handshaking() }
            pub fn handshake_kind(&self) -> Option<rustls::HandshakeKind> { self.inner.handshake_kind() }
            pub fn set_handshake_deadline(&mut self, deadline: Option<Instant>) { self.deadline = deadline; }
            pub fn next_timeout(&self) -> Option<Instant> {
                if self.inner.is_handshaking() && !self.timed_out { self.deadline } else { None }
            }
            ...
            pub fn handle_timeout(&mut self, now: Instant) -> Option<&'static str> {
                if self.next_timeout().is_some_and(|d| now >= d) {
                    self.timed_out = true; self.deadline = None; Some("ERR_TLS_HANDSHAKE_TIMEOUT")
                } else { None }
            }
        }
    }
}
endpoint!(Client, rustls::client::UnbufferedClientConnection, rustls::client::ClientConnectionData);
endpoint!(Server, rustls::server::UnbufferedServerConnection, rustls::server::ServerConnectionData);
...
pub fn node_error_code(error: &Error) -> &'static str {
    match error {
        Error::InvalidCertificate(CertificateError::Expired | CertificateError::ExpiredContext { .. }) => "CERT_HAS_EXPIRED",
        Error::InvalidCertificate(CertificateError::NotValidYet | CertificateError::NotValidYetContext { .. }) => "CERT_NOT_YET_VALID",
        Error::InvalidCertificate(CertificateError::NotValidForName | CertificateError::NotValidForNameContext { .. }) => "ERR_TLS_CERT_ALTNAME_INVALID",
        Error::InvalidCertificate(CertificateError::UnknownIssuer) => "UNABLE_TO_VERIFY_LEAF_SIGNATURE",
...
#[derive(Debug)]
struct Unverified { provider: Arc<rustls::crypto::CryptoProvider> }
