...
fn local_infile_enabled_is_an_explicit_borrowed_request() {
    ...
    c.query(1, "LOAD DATA LOCAL INFILE 'fixture.tsv'", None)
        .unwrap();
    flush(&mut c);
    c.receive(&frame(1, b"\xfbfixture.tsv")).unwrap();
    let Some(Event::LocalInfile { token, file_name }) = c.next_event().unwrap() else {
        panic!()
    };
    assert_eq!(token, 1);
    ...
    c.local_infile_finish().unwrap();
    assert_eq!(flush(&mut c), frame(3, b""));
    c.receive(&ok(4, 1, 2)).unwrap();
    let Some(Event::Ok { packet, .. }) = c.next_event().unwrap() else {
        panic!()
    };
