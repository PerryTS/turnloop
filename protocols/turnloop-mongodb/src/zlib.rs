...
//! message, and no allocation after the first.
//!
//! `OP_COMPRESSED` payloads are separate RFC 1950 streams, which normally means
//! `flate2::Compress::reset` between messages. That is free natively and is not
//! free on wasm32: miniz_oxide keeps the LZ code buffer on the heap there
//! (`deflate/core.rs`, `LZOxide::new`) and `CompressorOxide::reset` replaces it
//! wholesale, so one reset per message is one allocation per message on WASI and
//! on the web. DESIGN §3.3 forbids per-operation allocation in every
//! configuration, so nothing here resets.
//!
...
//! byte-aligned — followed by the big-endian Adler-32 of the message. The result
//! is a complete, independently decompressible stream per message, while the
