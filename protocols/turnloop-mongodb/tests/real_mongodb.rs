...
mod common;
use common::{Driver, ports, raw};
use std::time::{Duration, Instant};
use turnloop_mongodb::{
    Error,
    bson::{Document, doc, raw::RawDocument},
...
fn rows(reply: &Document) -> Vec<Document> {
    ...
    CursorBatch::parse(&r)
        ...
        .rows()
        .map(|d| {
            Document::try_from(d.expect("fixture operation must succeed"))
                .expect("fixture operation must succeed")
        })
        .collect()
}
fn crud(d: &mut Driver) {
    ...
    c.insert(db, coll, true, None)
        .expect("fixture operation must succeed");
    let result = command(d, &c, &[("documents", &refs)]);
    assert_eq!(
        result.get_i32("n").expect("fixture operation must succeed"),
        3
    );
    ...
    let result = d
        .raw_command(c.raw(), &[("documents", &refs)])
        .expect("fixture operation must succeed");
    let mut bulk = BulkResult::default();
    assert!(
        !bulk
            .accept(&raw(&result), 7, true)
            .expect("fixture operation must succeed")
    );
    assert_eq!(
        bulk.write_errors[0]
    ...
    let mut bulk = BulkResult::default();
    assert!(
        bulk.accept(&raw(&result), 0, false)
            .expect("fixture operation must succeed")
    );
    ...
    let mut id = batch.id;
    while id != 0 {
        c.get_more(db, coll, id, Some(1), None)
            .expect("fixture operation must succeed");
    ...
    assert_eq!(count, 4);
    c.find_one(db, coll, &raw(&doc! {"_id":2}))
        .expect("fixture operation must succeed");
    assert_eq!(
        rows(&command(d, &c, &[]))[0]
            .get_i32("x")
    ...
    let reply = command(d, &c, &[]);
    let r = raw(&reply);
    let id = CursorBatch::parse(&r)
        .expect("fixture operation must succeed")
        .id;
    assert_ne!(id, 0);
    c.kill_cursor(db, coll, id)
        .expect("fixture operation must succeed");
    let reply = command(d, &c, &[]);
    assert_eq!(
        reply
            .get_array("cursorsKilled")
            .expect("fixture operation must succeed")[0]
            .as_i64(),
        Some(id)
    );
    ...
    c.count_documents(db, coll, &empty, 0, 0)
        .expect("fixture operation must succeed");
    assert_eq!(
        rows(&command(d, &c, &[]))[0]
            .get_i32("n")
    ...
    c.estimated_document_count(db, coll, None)
        .expect("fixture operation must succeed");
    assert_eq!(
        command(d, &c, &[])
            .get_i32("n")
    ...
    c.distinct(db, coll, "x", &empty, None)
        .expect("fixture operation must succeed");
    assert_eq!(
        command(d, &c, &[])
            .get_array("values")
    ...
    c.list_indexes(db, coll)
        .expect("fixture operation must succeed");
    assert_eq!(rows(&command(d, &c, &[])).len(), 2);
    c.drop_index(db, coll, "x_1")
        .expect("fixture operation must succeed");
    assert_eq!(
        command(d, &c, &[])
            .get_i32("nIndexesWas")
    ...
    c.list_collections(db, true, &empty)
        .expect("fixture operation must succeed");
    assert_eq!(
        rows(&command(d, &c, &[]))[0]
            .get_str("name")
    ...
    let delete = model
        .delete(&raw(&doc! {"_id":1}), false, None)
        .expect("fixture operation must succeed");
    c.delete(db, coll, true, None)
        .expect("fixture operation must succeed");
    assert_eq!(
...
fn real_standalone_replica_scram_tls() {
    ...
    let mut d = Driver::connect(&uri(port("standalone"), "compressors=zlib"))
        .expect("fixture operation must succeed");
    crud(&mut d);
    ...
    user(&mut tls);
    let mut tls =
        Driver::connect(&uri(port("tls"), "tls=true")).expect("fixture operation must succeed");
    assert_eq!(
    ...
    user(&mut bootstrap);
    let mut d = Driver::connect(&uri(primary_port, "compressors=zlib"))
        .expect("fixture operation must succeed");
    ...
    eprintln!(
        "Verified standalone, 3-member replica set, SCRAM SHA-1/SHA-256/default, TLS, zlib, CRUD, cursors, transactions, retry identity and change stream events"
    );
}
fn transactions_and_changes(d: &mut Driver) {
    ...
    w.clear();
    w.string("insert", "items")
        .expect("fixture operation must succeed");
    session
        .decorate(&mut w, false)
        .expect("fixture operation must succeed");
    ...
    let out = d
        .raw_command(
            w.as_raw().expect("fixture operation must succeed"),
            &[("documents", &[&entry])],
        )
        .expect("fixture operation must succeed");
    Error::from_response(&raw(&out)).expect("fixture operation must succeed");
    ...
    let out = d
        .raw_command(w.as_raw().expect("fixture operation must succeed"), &[])
        .expect("fixture operation must succeed");
    Error::from_response(&raw(&out)).expect("fixture operation must succeed");
    assert_eq!(
    ...
    session
        .next_retryable_write()
        .expect("fixture operation must succeed");
    w.clear();
    w.string("update", "items")
        .expect("fixture operation must succeed");
    session
        .decorate(&mut w, true)
        .expect("fixture operation must succeed");
    ...
    let reply = d
        .run(db, doc! {"find":"items","filter":{"_id":1}})
        .expect("fixture operation must succeed");
    assert_eq!(
        rows(&reply)[0]
            .get_i32("x")
    ...
    d.run(db, doc! {"insert":"items","documents":[{"_id":3}]})
        .expect("fixture operation must succeed");
    let mut c = Command::new();
    c.get_more(db, "items", id, Some(1), Some(5000))
        .expect("fixture operation must succeed");
    let reply = command(d, &c, &[]);
    let r = raw(&reply);
    let batch = CursorBatch::parse(&r).expect("fixture operation must succeed");
    ...
    change
        .observe_document(event)
        .expect("fixture operation must succeed");
    assert!(change.resume_token.is_some());
    change
        .finish_batch(batch.post_batch_resume_token)
        .expect("fixture operation must succeed");
    c.kill_cursor(db, "items", id)
        .expect("fixture operation must succeed");
    command(d, &c, &[]);
    let resumed = d
        .run(
        ...
        .expect("fixture operation must succeed");
    let r = raw(&resumed);
    let resumed_id = CursorBatch::parse(&r)
        .expect("fixture operation must succeed")
        .id;
    ...
    c.kill_cursor(db, "items", resumed_id)
        .expect("fixture operation must succeed");
    command(d, &c, &[]);
}

fn retry_coordinator(d: &mut Driver) {
    use turnloop_mongodb::{ConnectionEvent, operation::*};
    let db = "lane_retry";
    d.run(db, doc! {"dropDatabase":1})
        .expect("fixture operation must succeed");
    d.run(db, doc! {"create":"items"})
    ...
    for kind in [OperationKind::Write, OperationKind::Read] {
        ...
        loop {
            match op.action() {
                OperationAction::Select { .. } => op
                    ...
                    .expect("fixture operation must succeed"),
                OperationAction::Checkout { .. } => {
                    op.checked_out().expect("fixture operation must succeed")
                }
                OperationAction::Send { .. } => {
                    op.send(&mut d.core, Instant::now())
                        .expect("fixture operation must succeed");
                    sends += 1;
                }
                    ...
                    if let Some(event) = d.core.poll_event() {
                        match event {
                            ConnectionEvent::Reply { token } => {
                                ...
                                if op
                                    .response(
                                        d.core.reply().expect("fixture operation must succeed"),
                                    )
                                    .expect("fixture operation must succeed")
                                {
                                    success = Some(
        ...
        if kind == OperationKind::Write {
            assert_eq!(
                reply.get_i32("n").expect("fixture operation must succeed"),
                1
            );
        } else {
    ...
    assert!(matches!(
        end.failed(&mut s, error, 27, &mut w)
            .expect("fixture operation must succeed"),
        EndAction::Retry
    ));
    Error::from_response(&raw(&d
    ...
    w.string("find", "items")
        .expect("fixture operation must succeed");
    s.decorate_causal_read(&mut w, Some("majority"))
        .expect("fixture operation must succeed");
    w.string("$db", db).expect("fixture operation must succeed");
    w.finish().expect("fixture operation must succeed");
    let found = d
