...
fn warmed_query_row_and_named_execute_allocate_nothing() {
    ...
    let mut complete = 0;
    let mut run = || {
        c.query(1, "SELECT 42", None).expect("fixture operation must succeed");
        c.consume_output(c.output().len()).expect("fixture operation must succeed");
        ...
        while let Some(e) = c.next_event().expect("fixture operation must succeed") {
            match e {
                Event::Row { mut row, .. } => {
                    assert_eq!(row.next().expect("fixture operation must succeed").expect("fixture operation must succeed"), Some(&b"42"[..]));
                    rows += 1;
                }
    ...
    c.execute(2, q, None).expect("fixture operation must succeed");
    c.consume_output(c.output().len()).expect("fixture operation must succeed");
    c.receive(&[b'1', 0, 0, 0, 4, b'2', 0, 0, 0, 4]).expect("fixture operation must succeed");
