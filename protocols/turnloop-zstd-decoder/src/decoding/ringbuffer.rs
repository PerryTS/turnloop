...
impl RingBuffer {
    ...
    fn reserve_amortized(&mut self, amount: usize) {
        ...
        // SAFETY: new_layout is a checked, nonzero layout; null is rejected before use.
        let new_buf = unsafe {
    ...
    pub fn get(&self, idx: usize) -> Option<u8> {
        if idx < self.len() {
            ...
            // SAFETY: idx was checked against initialized length and wrapped into capacity.
    ...
    fn data_slice_parts(&self) -> ((*const u8, usize), (*const u8, usize)) {
        ...
        (
            // SAFETY: head is in bounds, or zero with a dangling empty allocation.
    ...
    fn free_slice_parts(&self) -> ((*mut u8, usize), (*mut u8, usize)) {
        ...
        (
            // SAFETY: tail is in bounds, or zero with a dangling empty allocation.
    ...
    pub fn extend_and_fill(&mut self, fill_with: u8, fill_length: usize) {
        ...
        // SAFETY: reserve and free_slice_parts provide at least fill1 writable bytes.
        unsafe {
        ...
        if fill1 < fill_length {
            ...
            // SAFETY: fill2 is the remaining reserved capacity in the second free slice.
            unsafe {
    ...
    pub fn extend_from_reader<R: Read>(
        ...
        debug_assert!(len1 + len2 >= fill_length);
        let fill1 = usize::min(len1, fill_length);
        // SAFETY: initialize the reserved free region before exposing it to Read.
        let s1 = unsafe {
        ...
        if fill1 < fill_length {
            let fill2 = fill_length - fill1;
            debug_assert_eq!(fill_length, fill1 + fill2);
            // SAFETY: the second reserved region is disjoint from s1 and initialized here.
            let s2 = unsafe {
    ...
    pub unsafe fn extend_from_within_unchecked_branchless(&mut self, start: usize, len: usize) {
        // SAFETY: caller guarantees initialized source range and reserved destination;
        // the computed disjoint segments partition those ranges without exceeding them.
        unsafe {
...
impl Drop for RingBuffer {
    fn drop(&mut self) {
        ...
        // SAFETY: buf owns the nonzero allocation made with this exact layout.
        unsafe {
            dealloc(self.buf.as_ptr(), current_layout);
        }
    }
...
unsafe fn copy_bytes_overshooting(
    ...
    // SAFETY: caller supplies valid, disjoint source/destination regions with the
    // stated lengths. Each branch copies only within those regions.
    unsafe {
