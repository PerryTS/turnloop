...
with path.open('w') as output:
    output.write(json.dumps({'metadata': {'platform': platform.platform(), 'machine': platform.machine(), 'rounds': args.rounds, 'profile': 'release codegen-units=1', 'toolchain': 'nightly-2026-08-20'}}) + '\n')
    for row in rows:
